...
export default function StudioEnginePage() {
  ...
  // Immediate local cache hydration on client mount
  useEffect(() => {
  ...
  // Debounced Cloud & Local Auto-Save
  useEffect(() => {
  ...
  return (
    <main className="min-h-screen flex flex-col bg-[var(--cf-bg)] text-[var(--cf-text)] transition-colors duration-300">
      ...
      {/* Workspace Header with Quick Jump Back to Dashboard */}
      <div className="border-b border-[var(--cf-border-soft)] bg-[var(--cf-surface)] px-4 md:px-8 py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link
              href="/dashboard"
              className="inline-flex items-center gap-1.5 text-xs font-mono text-[var(--cf-text-muted)] hover:text-[var(--cf-accent)] transition-colors"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>Back to Executive Briefing</span>
            </Link>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 border border-emerald-500/20 font-bold hidden sm:inline-block">
              P20 Bedrock Math Active
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 md:px-8 py-10 space-y-10 lg:space-y-12">
          ...
          >
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-serif font-bold tracking-tight text-[var(--cf-text)]">
                    {isViewingSample ? 'Sample Simulation Active' : 'Calibrated Production Model'}
                  </span>
                </div>
                <p className="text-[11px] text-[var(--cf-text-muted)] max-w-xl leading-relaxed mt-0.5">
                  {isViewingSample
                    ? 'Using standard Alex Vance sample stream. Import your real CSVs or calibrate manually.'
                    : `Your sovereign records drive the engine. P20 baseline floor: ${currencySymbol}${calculation.floorIncome.toLocaleString()}/mo.`}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 relative z-10 self-end sm:self-auto">
              ...
              >
                ...
                <span>{isViewingSample ? 'Calibrate Numbers' : 'Recalibrate'}</span>
              </button>

              <button
                type="button"
                onClick={() => setIsPasteModalOpen(true)}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono border transition-all cursor-pointer bg-[var(--cf-surface)] border-[var(--cf-border)] text-[var(--cf-text)] hover:border-[var(--cf-text-muted)]"
              >
        ...
        <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-6 pt-6 hairline-t">
          <button
            ...
            className="text-xs flex items-center gap-1.5 transition-colors cursor-pointer text-[var(--cf-text-muted)] hover:text-[var(--cf-accent)]"
          >
          ...
          <div className="flex flex-wrap items-center justify-center sm:justify-end gap-3 sm:gap-4 w-full sm:w-auto">
            ...
            <button
              ...
              className="flex-1 sm:flex-initial justify-center flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg transition-colors cursor-pointer font-mono whitespace-nowrap border bg-[var(--cf-surface)] border-[var(--cf-border)] text-[var(--cf-text)]"
            >
        ...
        {showPhilosophy && (
          ...
          >
            ...
            <p>
              By computing the <strong>20th percentile income floor</strong>, CashFloor isolates the empirical baseline that was met or exceeded in 80% of all operating history. Budgeting for baseline personal living costs at this floor guarantees that lean months cause zero financial panic.
            </p>
          </motion.div>
        )}
      </div>

      <footer className="w-full py-6 px-4 md:px-8 mt-4 bg-[var(--cf-bg-deep)] border-t border-[var(--cf-border)]">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[var(--cf-text-faint)] text-xs">· Studio Calculation Engine</span>
          </div>
          <div className="flex items-center gap-4 text-[11px] font-mono text-[var(--cf-text-faint)]">
            <Link href="/dashboard" className="hover:text-[var(--cf-accent)] transition-colors">Dashboard</Link>
            <Link href="/daily" className="hover:text-[var(--cf-accent)] transition-colors">Daily Stream</Link>
            <Link href="/integrations" className="hover:text-[var(--cf-accent)] transition-colors">Integrations</Link>
            <Link href="/" className="hover:text-[var(--cf-text)] transition-colors">Home</Link>
          </div>
        </div>
      </footer>
