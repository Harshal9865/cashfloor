...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--cf-bg)] text-[var(--cf-text)] flex flex-col font-sans transition-colors duration-300">
      {/* Global Marketing Navigation Bar with full menu & user dropdown */}
      <MarketingNav />

      {/* Main Content */}
      <main className="flex-1 max-w-6xl mx-auto px-4 sm:px-6 md:px-8 pt-10 pb-24 space-y-16">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-6">
          ...
          <div className="pt-2 flex items-center justify-center gap-3">
            <span className={`text-xs font-medium ${!isAnnual ? 'text-[var(--cf-text)] font-bold' : 'text-[var(--cf-text-muted)]'}`}>
              Monthly Billing
            </span>
            ...
            <button
              ...
              className="relative w-14 h-7 rounded-full bg-[var(--cf-surface-alt)] border border-[var(--cf-border)] p-1 transition-colors focus:outline-none cursor-pointer"
              aria-label="Toggle annual billing"
            >
              <motion.div
                className="w-5 h-5 rounded-full bg-[var(--cf-accent)] shadow-sm"
                animate={{ x: isAnnual ? 26 : 0 }}
                transition={{ type: 'spring', stiffness: 500, damping: 30 }}
              />
            </button>

            <span className={`text-xs font-medium flex items-center gap-1.5 ${isAnnual ? 'text-[var(--cf-text)] font-bold' : 'text-[var(--cf-text-muted)]'}`}>
              <span>Annual Billing</span>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase bg-emerald-500 text-white shadow-xs">
                Save 25%
              </span>
            </span>
          </div>
          ...
          {PRICING_PLANS.map((plan, index) => (
            <motion.div
              ...
              className={`relative flex flex-col justify-between rounded-3xl p-8 border transition-all duration-300 ${
                plan.highlight
                  ? 'bg-[var(--cf-surface)] border-[var(--cf-accent)] shadow-2xl ring-2 ring-[var(--cf-accent)]/20 md:-translate-y-2'
                  : 'bg-[var(--cf-surface)] border-[var(--cf-border)] hover:border-[var(--cf-accent)]/40 shadow-sm hover:shadow-lg'
              }`}
            >
              {/* Highlight Badge */}
              {plan.highlight && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-3.5 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider bg-gradient-to-r from-[var(--cf-accent)] to-emerald-600 text-white shadow-md">
                  {plan.badge}
                </div>
              )}
