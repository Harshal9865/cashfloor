...
import { Plus, Trash2, Globe, Clock, FileText } from 'lucide-react';
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {onOpenInvoiceGenerator && (
            <button
              ...
              type="button"
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold text-white transition-opacity hover:opacity-90 cursor-pointer shadow-sm"
              style={{ background: 'var(--cf-accent)' }}
            >
              <FileText className="w-3.5 h-3.5" /> Generate Invoice (PDF)
            </button>
          )}
          <button
            onClick={handleAdd}
            disabled={isLocked}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold border transition-opacity hover:opacity-90 disabled:opacity-50 cursor-pointer"
        ...
        <div className="flex items-center gap-2">
          {onOpenInvoiceGenerator && (
            <button
              onClick={onOpenInvoiceGenerator}
              type="button"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] uppercase tracking-wider font-semibold transition-all hover:opacity-90 cursor-pointer text-white shadow-sm"
              style={{ background: 'var(--cf-accent)' }}
            >
              <FileText className="w-3 h-3" /> Create Invoice (PDF)
            </button>
          )}
          <button
            onClick={handleAdd}
